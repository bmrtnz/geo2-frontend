  ...
  Filter: {
    ...
    Pair: {
      addComparaison: (
        r: |
          '='|'>'|'>='|'<'|'<='|
          'contains'|'in'|'startswith'|'endswith'|'notcontains'|
          'isnull'|'isnotnull'|'<>',
        [k, v]: [string, any],
      ) => [k, r, v]
    },
    ...
    andMerge: (chunk: FilterChunk, filter: FilterChunk[]) =>
      [...filter, 'and', chunk],
    mergeIf: (condition: () => boolean, chunk: FilterChunk, filter: FilterChunk[]) =>
    ...
    andMergeIfValue: (chunk: FilterChunk, filter: FilterChunk[]) =>
        Api.Filter.Chunk.hasValue(chunk) ? [...filter, 'and', chunk] : filter,
  }
}
