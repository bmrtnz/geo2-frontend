...
export class StockListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.stockArticlesAgeService.customVariables.societe =
    ...
    this.detailedFields =
      this.stockArticlesAgeService.model.getDetailedFields(2);

    this.especes = this.stockArticlesAgeService.getFilterDatasource(
      "article.matierePremiere.espece.description"
    );
    this.origines = this.stockArticlesAgeService.getFilterDatasource(
      "article.matierePremiere.origine.description"
    );
    this.varietes = this.stockArticlesAgeService.getFilterDatasource(
      "article.matierePremiere.variete.description"
    );
    this.calibresUnifies = this.stockArticlesAgeService.getFilterDatasource(
      "article.matierePremiere.calibreUnifie.description"
    );
    this.calibresMarquages = this.stockArticlesAgeService.getFilterDatasource(
      "article.normalisation.calibreMarquage.description"
    );
    this.emballages = this.stockArticlesAgeService.getFilterDatasource(
      "article.emballage.emballage.description"
    );
    this.matieresPremieres = this.stockArticlesAgeService.getFilterDatasource(
      "article.matierePremiere.modeCulture.description"
    );
    this.categories = this.stockArticlesAgeService.getFilterDatasource(
  ...
  onFieldValueChange(event: string[], dataField: string) {
    ...
    const filters = Object.entries(this.tagFilters)
  ...
  onCustomFilterChange(items: any[], selector: string) {
    items.length
      ? (this.stockArticlesAgeService.customVariables[selector] = items.map(
          (value) => (delete value.__typename, value)
        ))
