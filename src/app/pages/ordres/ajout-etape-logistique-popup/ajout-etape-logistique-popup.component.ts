...
export class AjoutEtapeLogistiquePopupComponent {
  ...
  applyClick() {
    this.functionsService
      .fAjoutOrdlog(
        this.ligneId,
        this.groupageSB.value.id,
        this.lieuSB.value.id
      )
  ...
  onShowing(e) {
    e.component
      .content()
      .parentNode.classList.add("ajout-etape-logistique-popup");
    this.groupageDS = this.typesTiersService.getDataSource_v2(
      ["id", "description"],
      100
    );
    this.groupageDS.filter([
      ["valide", "=", true],
      "and",
      [["id", "=", "S"], "or", ["id", "=", "O"], "or", ["id", "=", "G"]],
    ]);
    this.lieupassageaquaiDS = this.lieupassageaquaiService.getDataSource_v2([
      "id",
    ]);
