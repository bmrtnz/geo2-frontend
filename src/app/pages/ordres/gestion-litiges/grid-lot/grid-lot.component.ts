...
export class GridLotComponent implements OnInit, OnChanges {
  ...
  public readonly byTemplateConfig = {
    "ligne.responsableNombrePalettes":
  ...
  public updateLot(data: Partial<LitigeLigne>) {
    return interval(100).pipe(
      ...
      concatMap((datasource) => {
        (datasource.items() as Partial<LitigeLigneFait>[]).forEach(
          (item, rowIndex) => {
  ...
  public calculateResponsableAvoir(rowData: Partial<LitigeLigneFait>) {
    return (
      rowData.ligne.responsableQuantite * rowData.ligne.responsablePrixUnitaire
    );
  }
...
export class MergeSiblingColumnsPipe implements PipeTransform {
  transform(input: {
    ...
    return htmlStructure.reduce(
      (accumulator, current) => accumulator + current.outerHTML,
      ""
    );
  }
}
