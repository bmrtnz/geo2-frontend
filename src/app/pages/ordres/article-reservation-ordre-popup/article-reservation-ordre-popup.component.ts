...
import {
  ...
  Reservation,
} from "../grid-reservation-stock/grid-reservation-stock.component";
...
export class ArticleReservationOrdrePopupComponent implements OnChanges {
  ...
  @ViewChild(GridReservationStockComponent)
  gridResa: GridReservationStockComponent;
  @ViewChild(GridReservationStockEnCoursComponent)
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.ordreLigne.currentValue !== undefined) {
      this.quantiteAReserver =
        changes.ordreLigne.currentValue.nombreColisCommandes;
      this.logQuantity();
    }
  ...
  setTitle() {
    this.titleStart = this.localizeService.localize(
      "title-start-article-reservation-ordre-popup"
    );
    if (this.ordreLigne) {
      this.ordreLigneInfo = this.ordreLigne;
      this.titleMid =
        "n° " +
        this.ordreLigne.ordre.numero +
        " / " +
        this.ordreLigne.ordre.entrepot.code;
      this.titleEnd = this.localizeService
        .localize("title-end-article-reservation-ordre-popup")
  ...
  resizePopup() {
    if (this.popupFullscreen)
      this.gridResa.datagrid.instance.option("grouping", {
        autoExpandAll: false,
      });
  ...
  onReservationChange([
    ...
    if (nombreReservations === 0)
      ...
      this.pushLog(
        `${nombreReservations} déstockage(s) effectué(s) sur ${fournisseur}`
      );
    this.gridResaEnCours.reloadSource(this.ordreLigne.id);
  ...
  private logOK() {
    ...
    if (this.logs.length > 1) {
      if (
        this.logs[this.logs.length - 2].indexOf("Quantité à déstocker") === 0
      ) {
  ...
  private updateNombreResa() {
    return this.ordreLignesService.fGetInfoResa(this.ordreLigneInfo.id).pipe(
      map((res) => {
        const qteDispo =
          res.data.fGetInfoResa.data.ll_tot_qte_ini -
          res.data.fGetInfoResa.data.ll_tot_qte_res;
