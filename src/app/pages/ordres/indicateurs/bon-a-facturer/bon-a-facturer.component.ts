...
export class BonAFacturerComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild("entrepotValue", { static: false })
  entrepotSB: DxSelectBoxComponent;
  @ViewChild("commercialValue", { static: false })
  commercialSB: DxSelectBoxComponent;
  @ViewChild("assistanteValue", { static: false })
  assistanteSB: DxSelectBoxComponent;
  @ViewChild("periodeValue", { static: false })
  periodeSB: DxSelectBoxComponent;
  @ViewChild("dateStartValue", { static: false })
  dateStartSB: DxSelectBoxComponent;
  @ViewChild("dateEndValue", { static: false })
  dateEndSB: DxSelectBoxComponent;
  ...
  updateFilters() {
    ...
    if (this.clientSB.value)
      this.entrepot.filter(["client.id", "=", this.clientSB.value.id]);
    ...
    if (this.entrepotSB.value)
      filters.push("and", ["entrepot.id", "=", this.entrepotSB.value.id]);
  ...
  updateAsBonAFacturer() {
    ...
    if (allOrdre.length)
      this.ordresService.saveAll({ allOrdre }).subscribe((res) => {
  ...
  setDates(e) {
    ...
    switch (periode) {
      ...
      case "Depuis 12 mois":
        ...
        fin = year + "-" + (month === 12 ? 1 : month + 1) + "-" + date;
        break;
      case "Mois dernier":
        temp =
          (month === 1 ? year - 1 : year) +
