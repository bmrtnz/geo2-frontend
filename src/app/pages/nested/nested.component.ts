...
export class NestedComponent implements OnDestroy {
    @ViewChild(GridNavigatorComponent, { static: true })
    gridNav: GridNavigatorComponent;
    ...
    onActivate(mainComponent: NestedMain & NestedPart) {
        ...
        const detailsOutlet = this.activatedRoute.children.find(
            ({ outlet }) => outlet === "details",
        );
        combineLatest([
    ...
    onActivateDetails(partComponent: NestedPart) {
        partComponent.contentReadyEvent
            .pipe(take(1))
            .subscribe(() =>
                this.gridNav.scrollToDetails({ behavior: "auto" }),
            );
