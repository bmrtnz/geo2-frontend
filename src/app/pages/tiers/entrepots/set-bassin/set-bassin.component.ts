...
export class SetBassinComponent implements OnInit, OnChanges {
  ...
  initializeGrid(id) {
    ...
    this.fetchDatasource(id).subscribe(
      (datasource) => (this.etbDatasource = datasource)
    );
  ...
  private bindSources(event: { component: dxDataGrid }) {
    ...
    GridConfiguratorService.bindLookupColumnSource(
      event.component,
      "transporteur.id",
      this.transporteursService.getLookupStore<Transporteur>(
        ["id", "raisonSocial", "ville"],
        "valide==true"
      )
    );
    GridConfiguratorService.bindLookupColumnSource(
      ...
      this.basesTarifService.getLookupStore<BaseTarif>(
        ["id", "description"],
        "valide==true"
      )
    );
    GridConfiguratorService.bindLookupColumnSource(
      ...
      this.devisesService.getLookupStore<Devise>(
        ["id", "description"],
        "valide==true"
      )
    );
  ...
  private updateBacSource(component: dxDataGrid) {
    const usedBacs: Partial<BureauAchat>[] = component
      .getDataSource()
      ?.items()
      .map(({ bureauAchat }) => bureauAchat);
    const bacFilter = usedBacs?.map(({ id }) => `id!="${id}"`).join(" and ");
    const bassListFilter = BASSINS.map((bassin) => `id=="${bassin}"`).join(
      " or "
    );
    const source = this.bureauxAchatService.getLookupStore<BureauAchat>(
      ["id", "raisonSocial"],
      `valide==true ${
        bacFilter ? "and " + bacFilter : ""
      } and (${bassListFilter})`
    );
    GridConfiguratorService.bindLookupColumnSource(
      component,
  ...
  private fetchDatasource(entrepotID: Entrepot["id"]) {
    return this.columns.pipe(
      map((columns) =>
        columns
          .map(({ dataField, calculateDisplayValue }) => [
            dataField,
            calculateDisplayValue,
          ])
          .flat(2)
      ),
      map((fields) => this.etbService.getDataSource_v2([...fields])),
      tap((datasource) =>
        datasource.filter([
          ["valide", "=", true],
          "and",
  ...
  public onInserting(event) {
    // complete row data
    event.data.entrepot = { id: this.route.snapshot.paramMap.get("id") } as {
      id: string;
    } & Partial<Entrepot>;
  ...
  public onOpened(elem) {
    ...
    let width = 0;
    if (elem.element.parentElement.classList.contains("smallDropDown"))
      width = 125;
    if (elem.element.parentElement.classList.contains("mediumDropDown"))
      width = 250;
    if (elem.element.parentElement.classList.contains("largeDropDown"))
      width = 500;
