...
describe("Model class", () => {
    ...
    describe("should handle getGQLFields()", () => {
        it("with defaults", (done) => {
            Entity.getGQLFields().subscribe((fields) => {
        ...
        it("with depth", (done) => {
            Entity.getGQLFields(2).subscribe((fields) => {
        ...
        it("with filter", (done) => {
            Entity.getGQLFields(1, /id$/).subscribe((fields) => {
        ...
        it("with filter and depth", (done) => {
            Entity.getGQLFields(1, /^(valide|type.typeDescription)$/, null, {
                forceFilter: true,
            }).subscribe((fields) => {
    ...
    describe("should handle getDetailedFields()", () => {
        it("with defaults", (done) => {
            Entity.getDetailedFields().subscribe((fields) => {
        ...
        it("with depth", (done) => {
            Entity.getDetailedFields(2).subscribe((fields) => {
                expect(fields.length).toEqual(7);
                expect(fields).toContain(
                    jasmine.objectContaining({
                        path: "type.stamp.id",
        ...
        it("with filter", (done) => {
            Entity.getDetailedFields(1, /id$/).subscribe((fields) => {
        ...
        it("with filter (forcedFilter options)", (done) => {
            Entity.getDetailedFields(1, /description$/, {
                forceFilter: true,
            }).subscribe((fields) => {
    ...
    describe("should handle getGraphQL", () => {
        it("should return graphQL query with GridBaseConfig", (done) => {
            ...
            Model.getGQLObservable(
                columns.map((property) => property.dataField),
            ).subscribe((result) => {
        ...
        it("should return graphQL query with string params", (done) => {
            Model.getGQLObservable([
                ...
                "moyenCommunication.description",
            ]).subscribe((result) => {
                expect(result).toMatch(
                    /nom\nprenom\nmoyenCommunication{\ndescription\nid\n}/,
                );
        ...
        it("should return empty string with empty array", (done) => {
            Model.getGQLObservable([]).subscribe((result) => {
