...
export class EntityCellTemplateComponent {
  ...
  public getSettings(dataField: string): ColumnSettings {
    const [, settings] = Object.entries(this.columnsSettings).find(
      ([k]) => k === dataField
    );
    if (!settings)
      throw Error(`No settings defined for dataField "${dataField}"`);
    return settings;
  }
  ...
  public onSelectBoxCellValueChanged(event, cell) {
    ...
    if (cell.setValue) {
      ...
      const value = displayExpression
        .map((key) =>
          Model.fetchValue(key.split(".").splice(1, 1), event.value)
        )
        .join(" - ");
    ...
    setTimeout(() => {
      if (cell.data?.proprietaireMarchandise) {
        ...
        cell.data.proprietaireMarchandise.listeExpediteurs =
          event.value.listeExpediteurs;
      }
    });
  }
...
export class CellDisplayPipe implements PipeTransform {
  constructor(private store: CellDisplayStore) {}
...
@NgModule({
  imports: [CommonModule, DxTextBoxModule, DxSelectBoxModule],
