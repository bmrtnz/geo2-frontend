    ...
    return new DataSource({
      store: this.createCustomStore({
        ...
        load: (options: LoadOptions) => {
          ...
          .pipe(
            map( res => this.asListCount(res.data.allConditionSpecial)),
            map( res => ({
              ...res,
              data: res.data.map( entity => new ConditionSpecial(entity))
            })),
            take(1),
          )
          .toPromise();
        ...
        byKey: (key) => {
          ...
          .pipe(
            map( res => new ConditionSpecial(res.data.conditionSpecial)),
            take(1),
          )
          .toPromise();
        },
