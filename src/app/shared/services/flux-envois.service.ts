...
export class FluxEnvoisService {
  ...
  prompt(fluxID: Flux["id"], ordreID: Ordre["id"], outputPopup: ConfirmationResultPopupComponent, annulation?: boolean) {
    ...
    return defer(() => {
      switch (fluxID) {
        case "ORDRE": {
          ...
          return this.envoisService.fConfirmationCommande(ordreID, societe.id, user.nomUtilisateur);
        }
        case "DETAIL":
          return this.envoisService.fDocumentEnvoiDetailsExp(ordreID, societe.id);
        case "MINI":
          return this.envoisService.fDocumentEnvoiConfirmationPrixAchat(ordreID);
        case "FICPAO":
