    ...
    return new DataSource({
      store: this.createCustomStore({
        ...
        load: (options: LoadOptions) => {
          ...
          type Response = { allColoration: RelayPage<Coloration> };
          const variables = {
            ...inputVariables,
          ...
          .pipe(
            map( res => this.asListCount(res.data.allColoration)),
            map( res => ({
              ...res,
              data: res.data.map( entity => new Coloration(entity))
            })),
            take(1),
          )
          .toPromise();
        ...
        byKey: (key) => {
          ...
          .pipe(
            map( res => new Coloration(res.data.coloration)),
            take(1),
          )
          .toPromise();
        },
