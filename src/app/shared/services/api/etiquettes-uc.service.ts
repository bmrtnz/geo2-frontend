      ...
      store: this.createCustomStore({
        ...
        load: (options: LoadOptions) =>
          new Promise(async (resolve) => {
            ...
            type Response = {
              allEtiquetteUc: RelayPage<EtiquetteUc>;
            };
            const variables = this.mapLoadOptionsToVariables(options);

            this.listenQuery<Response>(query, { variables }, (res) => {
              if (res.data && res.data.allEtiquetteUc)
                resolve(this.asInstancedListCount(res.data.allEtiquetteUc));
            });
          }),
        byKey: (key) =>
          new Promise(async (resolve) => {
            ...
            this.listenQuery<Response>(query, { variables }, (res) => {
              if (res.data && res.data.etiquetteUc)
                resolve(new EtiquetteUc(res.data.etiquetteUc));
            });
          }),
      }),
    });
