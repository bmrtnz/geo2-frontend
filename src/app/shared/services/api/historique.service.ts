...
enum HistoryModel {
  ...
  TRANSPORTEUR = "HistoriqueTransporteur",
}
...
enum HistoryModule {
  ...
  TRANSPORTEUR = "historique-transporteur",
}
...
export class HistoriqueService extends ApiService {
  ...
  saveByType(type: HistoryType, properties: OperationVariables) {
    const withLowerCaseFirst = ([first, ...rest]: string) =>
      [first.toLowerCase(), ...rest].join("");
    return from(import(`../../models/${HistoryModule[type]}.model`)).pipe(
