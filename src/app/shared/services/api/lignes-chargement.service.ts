...
export class LignesChargementService extends ApiService {
  ...
  getList(variables: QueryArgs, columns: Set<string>) {
    return this.apollo.query<{
      allLignesChargement: Partial<LigneChargement[]>;
    }>({
  ...
  saveAll(
    allLigneChargement: Partial<LigneChargement>[],
    columns: Set<string>
  ) {
  ...
  private buildDuplicateTransferGraph(operation: Operation, body: Set<string>) {
    return ApiService.buildGraph(
      "query",
      [
        {
          ...
          params: [
            ...
            {
              name: "originalOrdreId",
              value: "originalOrdreId",
              isVariable: true,
            },
