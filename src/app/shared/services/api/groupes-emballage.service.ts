    ...
    return new DataSource({
      ...
      store: this.createCustomStore({
        ...
        load: (options: LoadOptions) =>
          new Promise(async (resolve) => {
            type Response = {
              allDistinctGroupeEmballage: RelayPage<GroupeEmballage>;
            };
            ...
            this.listenQuery<Response>(query, { variables }, (res) => {
              if (res.data && res.data.allDistinctGroupeEmballage) {
                resolve(
                  this.asInstancedListCount(res.data.allDistinctGroupeEmballage)
                );
              }
            });
          }),
    ...
    return (key) =>
      new Promise(async (resolve) => {
        ...
        const res = await this.apollo
          .query<{ groupeEmballage: GroupeEmballage }>({
            query: gql(this.buildGetOneGraph(columns)),
            variables,
          })
          .toPromise();
        if (res?.data?.groupeEmballage)
          resolve(new GroupeEmballage(res.data.groupeEmballage));
      });
  }
}
