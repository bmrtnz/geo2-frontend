...
export class CertificationsModesCultureService extends ApiService implements APIRead {
      ...
      new Promise(async (resolve) => {
        ...
        this.listenQuery<Response>(query, { variables }, res => {
          if (res.data && res.data.certificationModeCulture)
            resolve(new CertificationModeCulture(res.data.certificationModeCulture));
        });
      });
  }
    ...
    return new DataSource({
      ...
      store: this.createCustomStore({
        load: (options: LoadOptions) => new Promise(async (resolve) => {
          ...
          type Response = { allCertificationModeCulture: RelayPage<CertificationModeCulture> };
          const query = await this.buildGetAll_v2(columns);
          const variables = this.mapLoadOptionsToVariables(options);
          this.listenQuery<Response>(query, { variables }, res => {
            if (res.data && res.data.allCertificationModeCulture) {
              resolve(this.asInstancedListCount(res.data.allCertificationModeCulture));
            }
          });
        }),
