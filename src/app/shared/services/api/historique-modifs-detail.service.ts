  ...
  implements APIRead
{
  ...
  private byKey(columns: Array<string>) {
    return (key) =>
      new Promise(async (resolve) => {
        ...
        type Response = {
          historiqueModificationDetail: HistoriqueModificationDetail;
        };
        const variables = { id: key };
        this.listenQuery<Response>(query, { variables }, (res) => {
          if (res.data && res.data.historiqueModificationDetail)
            resolve(
              new HistoriqueModificationDetail(
                res.data.historiqueModificationDetail
              )
            );
        });
      });
      ...
      store: this.createCustomStore({
        load: (options: LoadOptions) =>
          new Promise(async (resolve) => {
            ...
            type Response = {
              allHistoriqueModificationDetail: RelayPage<HistoriqueModificationDetail>;
            };
  ...
  save_v2(columns: Array<string>, variables: OperationVariables) {
    return this.apollo.mutate<{
      saveHistoriqueModificationDetail: HistoriqueModificationDetail;
    }>({
  ...
  public countModifDetailHistoryById(logistiqueId) {
    return this.countModifDetailHistory(
      this.mapDXFilterToRSQL(["logistique.id", "=", logistiqueId])
    );
  }
