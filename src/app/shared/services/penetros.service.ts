...
import { APIRead, ApiService, RelayPageVariables, RelayPage } from './api.service';
    ...
    return new DataSource({
      ...
      store: this.createCustomStore({
        ...
        load: (options: LoadOptions) => {
          ...
          .pipe(
            map( res => this.asListCount(res.data.allPenetro)),
            map( res => ({
              ...res,
              data: res.data.map( entity => new Penetro(entity))
            })),
            take(1),
          )
          .toPromise();
        ...
        byKey: (key) => {
          ...
          .pipe(
            map( res => new Penetro(res.data.penetro)),
            take(1),
          )
          .toPromise();
        },
