  ...
  getDates(e) {
    ...
    switch (periode) {
      ...
      case 'Depuis 30 jours':
        ...
        break;
      case 'Depuis 2 mois':
        deb = (month <= 2 ? year - 1 : year) + '-' + (month <= 2 ? 10 + month : month - 2) + '-' + date;
        fin = now;
        break;
      case 'Depuis 3 mois':
        deb = (month <= 3 ? year - 1 : year) + '-' + (month <= 3 ? 9 + month : month - 3) + '-' + date;
        fin = now;
        break;
      case 'Depuis 12 mois':
        ...
        break;
      case 'Mois dernier':
        temp = (month === 1 ? year - 1 : year) + '-' + (month === 1 ? 12 : month - 1);
        deb = temp + '-01';
        fin = temp + '-' + this.daysInMonth((month === 1 ? year - 1 : year), (month === 1 ? 12 : month - 1));
        break;
      case 'Mois en cours':
        temp = year + '-' + month;
        deb = temp + '-01';
      ...
      case 'Trimestre dernier':
        deb = (quarter === 1 ? year - 1 : year) + '-' + prevQuarterStart + '-01';
        fin = (quarter === 1 ? year - 1 : year) + '-' + (prevQuarterStart + 2) + '-'
              + this.daysInMonth((quarter === 1 ? year - 1 : year), prevQuarterStart + 2);
        break;
