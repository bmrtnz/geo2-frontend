...
export abstract class ApiService implements OnDestroy {
    ...
    static buildGraph(
        ...
        const mapOperations = (ops) =>
            ops.map(
                (o) =>
                    `${o.name}(${mapParams(o.params)}) {${mapPaths(
                        o?.body ?? [],
                    )}}`,
            );

        return `${type} ${alias}(${mapVariables(variables)}) { ${mapOperations(
            operations,
        )} }`;
    }
                ...
                if (typeof node[0] === "object") {
                    ...
                    if (selector === "this") {
                        const mappedFilter = Object.entries(value)
                            ...
                            .map((v) => JSON.parse(v));
                        currentFilter[index] =
                            this.mapDXFilterToRSQL(mappedFilter);
                    } else {
    ...
    public mapLoadOptionsToVariables(options: LoadOptions) {
        ...
        if (options.searchValue)
            // global search
            search.push(
                this.mapDXFilterToRSQL(this.mapDXSearchToDXFilter(options)),
            );
        ...
        if (options.totalSummary)
            variables.summary = options.totalSummary.map(
    ...
    >(options: Partial<LoadOptions>, cbk: (res: ApolloQueryResult<T>) => void) {
        ...
        const field = options.group.length
