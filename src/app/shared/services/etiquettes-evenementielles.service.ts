    ...
    return new DataSource({
      ...
      store: this.createCustomStore({
        ...
        load: (options: LoadOptions) => {
          ...
          .pipe(
            map( res => this.asListCount(res.data.allEtiquetteEvenementielle)),
            map( res => ({
              ...res,
              data: res.data.map( entity => new EtiquetteEvenementielle(entity))
            })),
            take(1),
          )
          .toPromise();
        ...
        byKey: (key) => {
          ...
          .pipe(
            map( res => new EtiquetteEvenementielle(res.data.etiquetteEvenementielle)),
            take(1),
          )
          .toPromise();
        },
